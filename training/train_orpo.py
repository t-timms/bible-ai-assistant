...
def main() -> None:
    ...
    parser.add_argument(
        "--run-name", type=str, default=None, help="W&B run name (default: <sft-folder>-orpo)"
    )
    parser.add_argument(
        "--preferences",
        type=str,
    ...
    parser.add_argument("--beta", type=float, default=0.1, help="ORPO odds ratio weight")
    parser.add_argument(
        "--no-wandb",
        action="store_true",
    ...
    pref_path = (
        ...
        if args.preferences
        else (project_root / "data" / "processed" / "preferences.json")
    )
    ...
    if args.no_wandb:
        wandb.init(project=wandb_project, name=run_name, mode="disabled")
    else:
        wandb.init(
    ...
    config = ORPOConfig(
        ...
        # warmup_steps: ~8% of total steps (~56 at default settings) — LR reaches peak quickly
        warmup_steps=5,
        logging_steps=10,
        eval_steps=20,
        eval_strategy="steps",
