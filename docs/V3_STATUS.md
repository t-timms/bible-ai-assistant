...
## ►►►►►►► EXTERNAL SOTA SWEEP DONE (2026-09-05) — not a clean sweep; task-metric SOTA holds, semantic doesn't

`scripts/run_external_baselines.sh` finally ran end-to-end for the first time in this
project's history — every prior attempt (this session and earlier) had died before
reaching a single GGUF-based comparator's eval step. **Six real, previously-undiscovered
bugs** were found and fixed along the way, each verified against real data/a real command
before moving on: (1) an extra sigmoid on top of an already-calibrated cross-encoder score
(protocol v5's own bug, caught before the sweep even started); (2) `huggingface-cli` fully
deprecated in the installed `huggingface_hub` — exits immediately, downloads nothing;
(3) a tab-delimiter field-collapse (bash treats runs of tabs as one IFS-whitespace
delimiter) that silently shifted every config field after two back-to-back empty ones —
fixed with `\x1f` as the separator; (4) two guessed GGUF filenames that 404'd (fixed by
querying `HfApi.model_info` directly instead of guessing); (5) `GGUF_DIR` was a relative
path, so `ollama create`'s `FROM` line got parsed as a registry pull against a literal host
named "models"; (6) the SERVED-tag existence check never accounted for ollama's
auto-appended `:latest` on locally-created models — failed **every** comparator
deterministically until traced past a wrong first guess (WSL flakiness) to the real cause.

**Result: 12 models scored (4 ours + 8 external; `qwen3-32b-instruct` skipped by decision —
CPU-offload, hours-scale, uncertain payoff)**. Full table + McNemar pairwise tests:
`docs/SOTA_EVAL.md` (regenerated by `scripts/sota_scoreboard.py`).

| | fuzzy (expo-excl) | semantic (expo-excl) | verse_quote exact | citation | hallucination |
|---|--:|--:|--:|--:|--:|
| **v3.2** | **0.500 (#1/12)** | 0.942 (#3/12) | **80.3%** | **98.9%** | **1.9%** |
| christian-bible-expert-12b (12B) | 0.490 | **0.946 (#1)** | 55.3% | 96.2% | 4.5% |
| qwen3-14b-instruct (14B, untuned) | 0.482 | 0.944 (#2) | 72.0% | 98.1% | 4.9% |
| rhema-gemmabible (4.5B) | 0.490 | 0.910 | 72.7% | 96.6% | 10.5% |

**The honest verdict, not rounded up:** v3.2 leads fuzzy (still misses the original 0.52 bar)
but ranks **#3 on semantic**, behind two larger models (12B, 14B) that were never tuned for
— or in the 12B's case, tuned but poorly, on — this task. Semantic alone rewards topical
correctness; it doesn't capture verbatim quote accuracy, citation grounding, or hallucination
avoidance, which is what this task actually needs. On *those* three metrics v3.2 beats both
larger models decisively (80.3% vs 55.3%/72.0% quote-exact; 98.9% vs 96.2%/98.1% citation;
1.9% vs 4.5%/4.9% hallucination). Paired McNemar on strict verse-accuracy: not distinguishable
from the 12B (p=0.503) but significantly ahead of the 14B (p=0.004).

**Conclusion:** v3.2 is the strongest model at this task's actual job — accurate, cited,
non-hallucinated scripture answers — of every model tested, at any size. It is **not** the
highest-scoring model on a generic semantic-similarity metric; two larger models edge it
there. Both are true. The "best open model at RAG-grounded scripture Q&A, size-independent"
claim (`docs/SOTA_EVAL.md`, `CLAUDE.md` scope) is supported on the task-specific metrics,
not on semantic alone — state it that way, not as an unqualified "beats everything."

Also found via web search while auditing the comparator set: `rhemabible/GemmaBible` (a
second, better-documented dedicated-bible model, added as a 9th external comparator) and
`AiForTheChurch/ChristianGPT` (Llama-2-7B, 2023, too stale to be worth adding).

**NEXT:** publish this finding in the model card / README with the qualified framing above;
decide whether `qwen3-32b-instruct` is worth running later (bigger risk of also beating
semantic, same expected task-metric pattern); ship v3.2 (unchanged from the prior block).
